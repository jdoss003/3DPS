...
#define	FCLK_FAST()		SPI_FCLK_FAST()	/* Set SPI clock for read/write (20MHz max) */
...
void power_on (void)
{
	...
	/* Configure MOSI/MISO/SCLK/SS pins */
...
void power_off (void)
{
	...
	/* De-configure MOSI/MISO/SCLK/SS pins (set hi-z) */
...
)
{
	...
	case ISDIO_READ:
		...
		if (send_cmd(CMD48, 0x80000000 | (DWORD)sdi->func << 28 | (DWORD)sdi->addr << 9 | ((sdi->ndata - 1) & 0x1FF)) == 0) {
			for (Timer1 = 100; (rc = xchg_spi(0xFF)) == 0xFF && Timer1; Timer1--) ;
			if (rc == 0xFE) {
